...
const moveToNextStatus = ref(false); // Toggle to move to next status after upload
...
const hasAnyClassification = (pkg) => {
    const c = hasClassification(pkg);
    return c.dangerous || c.fragile || c.oversized;
};
...
const showUploadInvoiceModal = (pkg) => {
    ...
    // Find the package in viewAllPackages to get the latest data - props.viewAllPackages is reactive
    const packagesArray = Array.isArray(props.viewAllPackages) ? props.viewAllPackages : [];
    const freshPackage = packagesArray.find(a => a.id === pkg.id);
    ...
    selectedItemIds.value = [];
    moveToNextStatus.value = false; // Reset toggle
...
const upload = async () => {
    ...
    // Only send status change flag if customer wants to move to next status
    if (moveToNextStatus.value) {
        formData.append("move_to_next_status", "1");
    }
    ...
    try {
        ...
        // Only redirect if customer chose to move to next status
        if (moveToNextStatus.value) {
            closeModal();
            router.visit(route("customer.suite.inReview"));
        } else {
            // Stay on current page and refresh to show updated invoice status
            // Use preserveState: false to ensure fresh data is loaded
            router.reload({ 
                only: ['viewAllPackages'],
                preserveState: false,
                preserveScroll: true,
                onSuccess: () => {
                    closeModal();
...
<template>
    ...
    >
        <div class="grid grid-cols-12 gap-4">
            <div class="col-span-9">
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td class="text-center">
                                <span v-if="action?.items?.length > 0" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-700">
                                    <i class="fas fa-box-open text-[10px]"></i>{{ action?.items?.length }}
                                </span>
                            ...
                            <td class="text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <span v-if="hasClassification(action).dangerous" class="w-5 h-5 rounded-full bg-red-100 flex items-center justify-center" title="Dangerous Goods">
                                        <i class="fas fa-fire text-red-600 text-[10px]"></i>
                                    </span>
                                    <span v-if="hasClassification(action).fragile" class="w-5 h-5 rounded-full bg-amber-100 flex items-center justify-center" title="Fragile">
                                        <i class="fas fa-wine-glass text-amber-600 text-[10px]"></i>
                                    </span>
                                    <span v-if="hasClassification(action).oversized" class="w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center" title="Oversized">
                                        <i class="fas fa-expand-arrows-alt text-blue-600 text-[10px]"></i>
                                    </span>
                                    <span v-if="!hasAnyClassification(action)" class="text-gray-300">—</span>
                                </div>
                            </td>
                            <td>
                                {{
                            ...
                            <td>{{ action?.total_weight }} {{ action?.weight_unit || 'lbs' }}</td>
                                <td>
                                    <p
                                        v-if="
                                            action?.status_name ==
                ...
                <!-- Empty State -->
                <div v-if="!viewAllPackages || viewAllPackages.length === 0" class="flex flex-col items-center justify-center py-16 px-8 text-center bg-white border border-t-0 rounded-b-lg">
                    <div class="w-20 h-20 mb-6 rounded-full bg-gray-100 flex items-center justify-center">
                        <i class="fas fa-inbox text-4xl text-gray-400"></i>
                    </div>
                    <h3 class="text-xl font-semibold text-gray-800 mb-2">Your Suite is Empty</h3>
                    <p class="text-gray-500 max-w-sm mb-6">
                        Ship packages to your suite address to see them here. Your unique suite number is displayed above.
                    </p>
                    <p class="text-sm text-gray-400">
                        <i class="fas fa-info-circle mr-1"></i> Use the address panel at the top-right to get your shipping address
                    </p>
                </div>
            </div>
    ...
    <Modal :show="isShowUploadInvoiceModal" @close="closeModal">
        <div class="p-6 space-y-4">
            ...
            <!-- Move to Next Status Toggle -->
            <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input 
                        type="checkbox" 
                        v-model="moveToNextStatus"
                        class="rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                    />
                    <div>
                        <span class="text-sm font-medium text-gray-700">Move to "In Review" status after upload</span>
                        <p class="text-xs text-gray-500 mt-1">
                            Check this if you've finished uploading all invoices. Leave unchecked to upload more invoices for other items.
                        </p>
                    </div>
                </label>
            </div>
